...
class SkillLib:
    ...
    def place(self, obj: str):
        ...
        try:
            self.estim.reset_obj(f"./GMatch/cache/{obj}.pt")
            err_code, err_msg = self.estim.run_once()
            if err_code != 0:
                raise RuntimeError("Pose estimation failed: " + err_msg)
            rospy.sleep(0.1)
            log += "Pose estimation done.\n"
...
class VLMAgent:
    ...
    def agent(self, task_descr: str, max_rounds: int = 10):
        ...
        for i in range(max_rounds):
            ...
            # Update history
            # Assistant's response (the plan)
            assistant_response = "; ".join([" ".join(c) for c in cmds])
            history.append({"role": "assistant", "content": assistant_response})
            # Next user prompt with execution log
            prompt_nxt = f"<log>\n{exec_log.strip()}\n</log>\nAnalyze the execution result and re-plan commands to finish the task (e.g. retry if the last execution failed, otherwise carry on). If the task is finished, return an empty string."
            history.append({"role": "user", "content": prompt_nxt})
            print(f"[user]\n{_indent(prompt_nxt)}")

            time.sleep(5)  # wait for a while before next round
        print("\n----- Task Finished -----")
...
if __name__ == "__main__":
    ...
    planner.agent("put insect repellent on the realsense box")
